...
if(isset($_GET['ID'])){
	if(isset($_GET['action']) && $_GET['action'] == "read") {
		$updateSQL = sprintf("UPDATE mortgage_advice SET READ_STATUS='Y' WHERE ID=%s",						
							   GetSQLValueString($_GET['ID'], "int"));
	} else if (isset($_GET['action']) && $_GET['action'] == "delete") {
		$updateSQL = sprintf("DELETE from mortgage_advice WHERE ID=%s",						
							   GetSQLValueString($_GET['ID'], "int"));
	}
//echo $updateSQL;
    if ($updateSQL != null) 
...
$query_queriesRS = "SELECT * FROM mortgage_advice_view ORDER BY CREATION_DATE_MA DESC";
...
<?php include('top.php')?>
    <div >
   <div class="orangehead">Mortgage Queries</div>
...
<table cellspacing="0" cellpadding="0" border="1px" bordercolor="#666666" >
          <tr>
            ...
            <th style="width:130px;" class="greentableheading"><span class="greentableheading">Surname, Firstname</span></th>
            <th style="width:160px; text-align:center;" class="greentableheading"><span class="greentableheading">Contact</span></th>
  ...
  <tr  <?php if(($i%2) == 1){  ?>class="even"<?php } else {?> class="odd"<?php } ++$i;?> >
    <td <?php echo $style; ?>><?php echo fromDBDate($row_queriesRS['CREATION_DATE_MA']); ?></td>
    <td <?php echo $style; ?>>
	...
	Prefered Phone :- <?php echo $row_queriesRS['PREF_NO_DESCRIPTION'];   ?><br />
    Prefered Time  :- <?php echo $row_queriesRS['TIME'];   ?><br />
...
Postcode :- <?php echo $row_queriesRS['POSTCODE_MA']; ?><br />
    </td>            
    <td>
    	<?php if($row_queriesRS['READ_STATUS'] == "N"){?> <a href="adminmortgagequeries.php?action=read&ID=<?php echo $row_queriesRS['ID']?>"><input type="button"  class="button" value="Mark Read"/></a><?php }?>
    	<br />
    	<a onclick="return confirm ('Are you sure you want to delete this record?')" href="adminmortgagequeries.php?action=delete&ID=<?php echo $row_queriesRS['ID']?>"><input type="button"  class="button" value="Delete"/></a>
    </td>
    </tr>
