			...
			function formSubmit(){
				...
				if(!$('label.'+opt.invalidClass+',label.'+opt.emptyClass,form).length){
					$.ajax({
						...
						,data:{
							form_contactform_name:getValue($('label.name input'))
							,form_contactform_email:getValue($('label.email input'))
							,form_contactform_phone:getValue($('label.phone input'))
							,form_contactform_fax:getValue($('label.fax input'))
							,form_contactform_state:getValue($('label.state input'))
							,form_contactform_message:getValue($('label.message textarea'))
							,form_contactform_captcha_code:getValue($('label.captcha input'))
							,MM_insert:getValue($('input.formname'))
							,owner_email:opt.ownerEmail||'#'
							,stripHTML:opt.stripHTML
						}
						,success: function(e){	
							document.getElementById('captcha_form_contactform').src = 'captcha/securimage_show_small_4letters.php?' + Math.random()						
							if (e.indexOf('captcha_error') > -1) {
